...
#import "SLPullAnimationHeader.h"
#import "Masonry.h"

@interface SLPullAnimationHeader ()

@property (strong, nonatomic) UIImageView *imageV;          // imageView
@property (assign, nonatomic) CGFloat headerViewHeight;     // headerView 高度
@property (assign, nonatomic) CGFloat screenWidth;

@end

@implementation SLPullAnimationHeader
- (UIImageView *)imageV {
    if (!_imageV) {
        _imageV = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ico_Default"]];
        _imageV.contentMode = UIViewContentModeScaleAspectFit;
        _imageV.userInteractionEnabled = YES;
        [_imageV addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(imageTapAction:)]];
    }
